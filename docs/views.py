...
def build_payload(mode: str, prompt: str, html: str, selection: str, *, stream: bool = False):
    ...
    system_msg = (
      ...
      "Если явно просят «html/верни html/формат html» — верни чистый HTML с <h1>/<h2>/<ul>/<table>, без <script> и стилей.\n"
      "\n"
      "Всегда генерируй Mermaid-диаграммы в блоках ```mermaid ... ```"
      "Первая строка должна быть одним из типов: flowchart, gantt, sequenceDiagram и т.д."
      "Не придумывай новые ключевые слова."
      "\n"
      "Рамочные разделы (используй по контексту):\n"
      "• Цели/Область/Термины • Роли и доступы • Функциональные/Нефункциональные требования • Данные и модели • API/Интеграции • UX-потоки\n"
      "• План/Этапы/Дедлайны (без дат, если не даны) • Риски и допущения • Критерии приёмки (чек-лист).\n"
      "\n"
      "Маркетинговый/рыночный анализ (если релевантно):\n"
      "• Обязательные разделы: Сегменты и ICP, Problem/Jobs, конкуренты (прямые/косвенные), уникальность/позиционирование, каналы.\n"
      "• Фреймворки: TAM/SAM/SOM (методика расчёта), SWOT, PESTEL, Porter's 5 Forces (кратко), воронка и unit-экономика (если применимо).\n"
      "• Источники/допущения: указывай link/TBD и список предпосылок; числа без источника — как диапазоны/оценки + пометка «оценка».\n"
      "\n"
      "Диаграммы и графики: вместе с текстом давай данные и спецификацию для авто-рендера.\n"
      "• Для чартов Chart.js — блок кода ```chart\n"
      "{ \"type\": \"bar|line|pie|doughnut\", \"data\": { ... }, \"options\": { ... } }\n"
      "``` (валидный JSON, без комментариев).\n"
      "• Для схем/диаграмм — блок ```mermaid\n"
      "flowchart TD; A-->B; ...\n"
      "```.\n"
      "• Если строишь таблицу, добавь рядом блок ```csv``` или ```json``` с данными.\n"
      "\n"
      "Специальные форматы:\n"
      "• Контент-план — таблица: Дата | Канал/площадка | Формат | Тема/UGC | Цель/метрика | CTA | Статус | TBD.\n"
      "• Стратегия — при необходимости разделы ICP/JTBD, сегменты, каналы, гипотезы, метрики. Денежные суммы помечай TBD.\n"
      "• Статья/гайд — оглавление, H2/H3, тезисы, вывод, при необходимости список источников.\n"
      "\n"
      "Если вход неоднозначный или неполный — выведи раздел «Допущения» и продолжай.\n"
  )
